...
# Chuyển Bare_Nuclei về dạng numeric
...
Sử dụng k=5 train trên tập train
```{r , echo=FALSE}
...
Tìm siêu tham số k dựa vào tập train và validation :
```{r , echo=FALSE}
i=1
k.optm=1

for ( i in 1:28){
  knn.mod <- knn(train = data.train[,1:9], 
                      test = data.validation[,1:9], 
                      cl= data.train$OutCome ,
                      k=i )
  
  k.optm[i]<-100*sum(data.validation$OutCome==knn.mod)/NROW(data.validation)
  k=i
  cat(k,'=',k.optm[i],'\n')
}
...
Kết quả tốt nhất tại k=3, Accuracy=99.27007% 

Kết quả sau khi huấn luyện trên tập train và validation với k=3 :
```{r , echo=FALSE}
# train và test lại
